...
function assignment(r: Language, rhs: P.Parser<ObjectTerm | Dot | Placeholder>) {
  return P.seqObj<Assignment>(['attribute', r.ObjectTerm], r.equals, ['value', rhs]).thru(
    type('assignment'),
  )
}
...
function specifier(
  r: Language,
  opening: P.Parser<string>,
  closing: P.Parser<string>,
  typeName: SpecifierType,
): P.Parser<Specifier> {
  return P.seqObj<Assignments>(
    opening,
    ['assignments', P.sepBy<Assignment, string>(r.AssignmentWithPlaceholder, r.comma)],
    closing,
  ).thru(type(specifierType(typeName)))
}

function specifierExpression(r: Language, typeName: SpecifierExpressionType) {
  return P.seqObj<{ lhs: SpecifierTerm; rhs: SpecifierTerm }>(
...
function type<T extends ParseResult, U extends T>(
  typeName: U['type'],
): (parser: P.Parser<T>) => P.Parser<U> {
  return (parser) => {
...
const MARPL = P.createLanguage<MARPL>({
  ...
  ObjectTerm: (r: Language) => {
    return P.alt(r.ObjectVariable, r.ObjectLiteral)
  },
  ObjectTermOrLiteral: (r: Language) => {
    return P.alt(r.literalExpression, r.ObjectTerm)
  },
  ...
  FunctionTerm: (r: Language) => {
    return P.seqObj<FunctionTerm>(
      ['name', P.regexp(/[a-zA-Z]\w*/)],
      r.openingParenthesis,
      [
        'arguments',
        P.sepBy<Argument, string>(P.alt(r.ObjectLiteral, r.SetLiteral, r.someVariable), r.comma),
      ],
      r.closingParenthesis,
  ...
  Dot: (r: Language) => {
    return P.seqObj<Dot>(['fromSpecifier', r.SetVariable], r.dot, ['item', r.ObjectLiteral]).thru(
      type('dot'),
    )
  },
  Placeholder: () => {
  ...
  AssignmentWithPlaceholder: (r: Language) => {
    return assignment(r, P.alt(r.ObjectTerm, r.Placeholder))
  },
  DottedAssignment: (r: Language) => {
    return assignment(r, P.alt(r.Dot, r.ObjectTerm, r.Placeholder))
  },
  ...
  SpecifierTerm: (r: Language) => {
    return P.alt(r.SpecifierExpression, r.Specifier)
  },
  SpecifierAtom: (r: Language) => {
    return P.seqObj<SpecifierAtom>(['set', r.SetVariable], r.colon, [
      'specifier',
      r.SpecifierTerm,
    ]).thru(type('specifier-atom'))
  },
  Rule: (r: Language) => {
    return P.seqObj<Rule>(['body', r.Body], r.arrow, ['head', r.Head]).thru(type('rule'))
  },
  Head: (r: Language) => {
    return P.alt(
  ...
  Body: (r: Language) => {
    return P.sepBy<RelationalAtom, string>(
      P.alt(r.RelationalAtom, r.SimpleRelationalAtom, r.SpecifierAtom),
      r.comma,
    )
  },
})
...
function rewrite(ast: Rule) {
      ...
      if (isSomeVariable(argument)) {
        ...
        for (const atom of ast.body) {
          if (isRelationalAtom(atom)) {
            ...
            if (isSetVariable(annotation) && annotation.name === name) {
              ast.head.annotation.arguments[arg].type = 'set-variable'
              continue arguments
            }

            if (atom.predicate.type === 'variable' && atom.predicate.name === name) {
              ast.head.annotation.arguments[arg].type = 'variable'
              continue arguments
            }

            for (const property of atom.arguments) {
              if (property.type === 'variable' && property.name === name) {
                ast.head.annotation.arguments[arg].type = 'variable'
                continue arguments
              }
            }
  ...
  for (const [idx, atom] of ast.body.entries()) {
    if (isRelationalAtom(atom) && isSpecifier(atom.annotation)) {
      const variable = { type: 'set-variable', name: `?_body_spec_for_${atom}` } as const
...
function specifierAtom(set: SetVariable, specifier: SpecifierTerm): SpecifierAtom {
  return { type: 'specifier-atom', set, specifier }
}

function maybeParse<T>(parser: P.Parser<T>, str: string) {
  try {
    parser.tryParse(str)
  } catch (_err) {
    return false
