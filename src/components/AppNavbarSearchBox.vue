<template>
  ...
  >
    <template slot="append">
      <b-button size="sm" @click="onEntitySearch(entitySearch)"
...
<script lang="ts">
  ...
  export default class AppNavbarSearchBox extends Vue {
    ...
    private searchEntities = _.debounce(async function (
      this: AppNavbarSearchBox,
      search,
    ) {
      try {
    ...
    private workAroundFilter(htmlText: string) {
      ...
      return htmlText
        .replace(
          TYPEAHEAD_WORKAROUND_REGEX,
          '$2 <small>$1</small><br /><small>$3</small>',
        )
        .trim()
    }
    ...
    private onSelectedEntity(event: any) {
      router.push({ name: 'entity', params: { id: event.id } })
      const searchBox: { inputValue: string } = this.$refs[
        'navbar-search-box'
      ] as any
      searchBox.inputValue = searchBox.inputValue
        .replace(TYPEAHEAD_WORKAROUND_REGEX, '$2')
        .trim()
    }
  }
</script>
