...
angular.module('util').factory('ruleExamples', [
    ...
    function(wikidataapi, util, i18n, sparql, $q, $http, ruleParser) {
        ...
        var rules = [
            '?x.P1696=?y@?S -> ?y.P1696=?y@F(?S)',
            '?x.P1696=?y@?S -> ?y.P1696=?y@?T', // x inverseOf y -> y inverseOf x
            '?x.P361=?y@?S -> ?y.P527=?x@?T', // x partOf y -> y hasPart x
            '|_ ?att: ?start _|(?S), ?x.P26=?y@?S -> ?y.P26=?x@{?att: ?start}', // x spouse y [att:start] -> y spouse x[att:start]
            '(P580: ?start)#?S, ?x.P26=?y@?S -> ?y.P26=?x@?S', // start=?start in ?S, ?x spouse ?y: ?S -> ?y spouse ?x: ?S
            '[P580: ?start](?S), ?x.P26=?y@?S -> ?y.P26=?x@?S', // ?S=[start=?start], ?x spouse ?y: ?S -> ?y spouse ?x: ?S
            '|_ _|(?S), ?x.P26=?y@?S -> ?y.P26=?x@?S', // ?S=[...], ?x spouse ?y: ?S -> ?y spouse ?x: ?S
