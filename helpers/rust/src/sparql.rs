...
pub(crate) fn query(query: &str) -> Result<Response> {
    log::trace!(target: "sparql", "query {:?}", query);
    ...
    log::trace!(target: "sparql", "query {:?}, got response {:?}", query, response);
    response
}
...
mod test {
    ...
    fn simple_query() {
        ...
        log::debug!(target: "sparql", "response: {}", response.status());
        assert_eq!(response.status(), 200);
        let mut reader = csv::Reader::from_reader(response);
        let result: Vec<Result> = reader.deserialize().flatten().collect();
        log::debug!(target: "sparql", "response: {:?}", result);
        assert_eq!(
            result,
            vec![
                Result {
